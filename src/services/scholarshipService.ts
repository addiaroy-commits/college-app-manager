 ...
 */
export async function searchExternalScholarships(
  query: string,
): Promise<any[]> {
 ...
 */
export function importScholarshipsFromCSV(data: string): any[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!name) continue; // skip rows without a scholarship name
 ...
 */
export async function verifyScholarshipLink(
  url: string,
