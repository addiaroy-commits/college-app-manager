...
import type { Scholarship } from "../stores/scholarshipStore";
 ...
 *   TOTAL (capped) .................. 100
 */
export function calculateMatchScore(scholarship: Scholarship): number {
  let score = 0;
  ...
  if (scholarship.minGpa > 0) {
    const gpaEntries = bragItems.filter((item: any) => item.category === "gpa");
  ...
  if (scholarship.majorEligibility && scholarship.majorEligibility.trim()) {
    const topPicks = readJson<any[]>(getUserKey("top-picks"), []);
    const userMajors = topPicks.map((p: any) =>
      (p.majorName ?? "").toLowerCase(),
    );
    const scholarshipMajor = scholarship.majorEligibility.trim().toLowerCase();

    const hasMatch = userMajors.some(
      (um: string) =>
        um.includes(scholarshipMajor) || scholarshipMajor.includes(um),
    );
  ...
  if (scholarship.schoolEligibility && scholarship.schoolEligibility.trim()) {
    ...
    const scholarshipSchool = scholarship.schoolEligibility
      .trim()
      .toLowerCase();
    const match = collegeStore.colleges.some(
      (c) =>
        c.name.toLowerCase().includes(scholarshipSchool) ||
  ...
  const userActivities = bragItems.filter((item: any) =>
    activityCategories.includes(item.category),
  );
  ...
  if (userActivities.length > 0) {
    ...
    if (mappedCategory) {
      const hasMatchingActivity = userActivities.some(
        (item: any) => item.category === mappedCategory,
      );
      if (hasMatchingActivity) score += 10;
    } else {
...
export function getMatchBadges(scholarship: Scholarship): string[] {
  const badges: string[] = [];
...
export function getScamWarnings(scholarship: Scholarship): string[] {
  ...
  if (scholarship.applicationLink) {
    ...
    for (const pattern of SUSPICIOUS_DOMAINS) {
      if (link.includes(pattern)) {
        warnings.push(
          `Suspicious link detected (contains "${pattern}") — verify before applying`,
        );
        break;
      }
    }
  ...
  if (!scholarship.deadline && scholarship.awardAmount === 0) {
    warnings.push(
      "Missing deadline and award amount — this may not be a real opportunity",
    );
  }
  ...
  if (nameLower.includes("guaranteed") || nameLower.includes("win")) {
    warnings.push(
      'Scholarship name contains "guaranteed" or "win" — be cautious of scams',
    );
  }
