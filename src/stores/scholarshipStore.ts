...
export type ScholarshipType =
  ...
  | "Community Service"
  | "Other";
export type ChecklistStatus = "Not Needed" | "Needed" | "In Progress" | "Done";
...
export interface ScholarshipEssayLink {
  id: string;
  essayId: string;
  essayTitle: string;
}
...
export interface Scholarship {
  ...
  gradeLevel: string;
  majorEligibility: string;
  schoolEligibility: string;
  applicationLink: string;
  notes: string;
  checklist: ChecklistItem[];
  docLinks: ScholarshipDocLink[];
  essayLinks: ScholarshipEssayLink[];
  tags: string[];
...
export interface ScholarshipGoal {
  id: string;
  title: string;
  type:
    | "apply-count"
...
const checklistDefaults: ChecklistItem[] = [
...
export const useScholarshipStore = defineStore("scholarships", () => {
  ...
  const goals = ref<ScholarshipGoal[]>([]);

  (function load() {
    const saved = localStorage.getItem(getUserKey("scholarships"));
    if (saved) scholarships.value = JSON.parse(saved);
    const savedGoals = localStorage.getItem(getUserKey("scholarship-goals"));
    if (savedGoals) goals.value = JSON.parse(savedGoals);
  })();

  function save() {
    localStorage.setItem(
      getUserKey("scholarships"),
      JSON.stringify(scholarships.value),
    );
    localStorage.setItem(
      getUserKey("scholarship-goals"),
      JSON.stringify(goals.value),
    );
  }

  function addScholarship(scholarship: Scholarship) {
    scholarships.value.push(scholarship);
    save();
  }

  function updateScholarship(id: string, updated: Scholarship) {
  ...
  function addGoal(goal: ScholarshipGoal) {
    goals.value.push(goal);
    save();
  }

  function updateGoal(id: string, updated: ScholarshipGoal) {
  ...
  function getChecklistDefaults(): ChecklistItem[] {
    return checklistDefaults.map((item) => ({ ...item }));
  }

  function seedScholarships() {
    if (scholarships.value.length > 0) return;
    ...
    const sampleScholarships: Scholarship[] = [
      {
        ...
        notes:
          "150 Coca-Cola Scholars selected annually. Leadership and service emphasis.",
        checklist: getChecklistDefaults().map((c) => {
          if (
            ...
            return { ...c, status: "In Progress" as ChecklistStatus };
          if (c.label === "CSS Profile Required" || c.label === "Portfolio")
            return { ...c, status: "Not Needed" as ChecklistStatus };
          return c;
        }),
        docLinks: [],
        essayLinks: [],
      ...
      },
      {
        ...
        notes:
          "Full cost of attendance. 300 scholars selected. Pell-eligible required.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "FAFSA Required" || c.label === "Financial Documents")
            return { ...c, status: "Needed" as ChecklistStatus };
      ...
      },
      {
        ...
        notes:
          "GPA requirement is a 2.5. Employees and their families eligible too.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio" || c.label === "CSS Profile Required")
            return { ...c, status: "Not Needed" as ChecklistStatus };
          return c;
      ...
      },
      {
        ...
        notes:
          "Pell Grant eligible required. Emphasis on overcoming adversity. $20,000 over 6 years plus laptop.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "FAFSA Required" || c.label === "Financial Documents")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (c.label === "Portfolio" || c.label === "CSS Profile Required")
      ...
      },
      {
        ...
        notes:
          "500 four-year scholarships. Judged on scholarship, leadership, and financial need.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio" || c.label === "CSS Profile Required")
            return { ...c, status: "Not Needed" as ChecklistStatus };
          return c;
      ...
      },
      {
        ...
        notes:
          "Critical financial need required ($65,000 or less household income). Overcome adversity.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "FAFSA Required" || c.label === "Financial Documents")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (c.label === "Portfolio" || c.label === "CSS Profile Required")
      ...
      },
      {
        ...
        notes:
          "Minority students. Leadership, community service, and financial need considered. Includes mentoring and career guidance.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio" || c.label === "CSS Profile Required")
            return { ...c, status: "Not Needed" as ChecklistStatus };
          return c;
      ...
      },
      {
        ...
        notes:
          "$40,000 over 4 years. African American students. Academic excellence and leadership.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio" || c.label === "CSS Profile Required")
            return { ...c, status: "Not Needed" as ChecklistStatus };
          return c;
      ...
      },
      {
        ...
        notes:
          "Significant project in STEM or humanities required. Awards from $10,000 to $50,000.",
        checklist: getChecklistDefaults().map((c) => {
      ...
      },
      {
        ...
        notes:
          "Based on PSAT/NMSQT scores from junior year. Entry is automatic via PSAT.",
        checklist: getChecklistDefaults().map((c) => {
          if (
            c.label === "Interview" ||
            c.label === "Portfolio" ||
      ...
      },
      {
        ...
        notes:
          "Local chapter scholarship. Check your local Rotary club for availability.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "CSS Profile Required" || c.label === "Portfolio")
            return { ...c, status: "Not Needed" as ChecklistStatus };
          return c;
      ...
      },
      {
        ...
        notes:
          "Audio essay contest. Record a 3-5 minute essay on a patriotic theme. National first prize $30,000.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Essay")
            return { ...c, status: "In Progress" as ChecklistStatus };
          if (
      ...
      },
      {
        ...
        notes:
          "Write an essay on one of several topics about Atlas Shrugged. $10,000 top prize. Multiple winners.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Essay")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (
      ...
      },
      {
        ...
        notes:
          "20 awards of $20,000. Sports participation plus leadership and community service.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio")
            return { ...c, status: "Needed" as ChecklistStatus };
      ...
      },
      {
        ...
        notes:
          "Submit original art or writing. National medalists receive scholarship opportunities.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (
      ...
      },
      {
        ...
        notes:
          "Video submission (2 minutes) about passion. No GPA or test score requirements.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (
      ...
      },
      {
        ...
        notes:
          "750-word essay on genetics topic. $1,000 for first place. Teacher sponsorship required.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Essay")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (
      ...
      },
      {
        ...
        notes:
          "Recognizes volunteer service. State honorees get $1,000, national honorees $5,000 plus D.C. trip.",
        checklist: getChecklistDefaults().map((c) => {
          if (c.label === "Portfolio")
            return { ...c, status: "Needed" as ChecklistStatus };
          if (c.label === "CSS Profile Required")
