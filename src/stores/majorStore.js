...
export const useMajorStore = defineStore("majors", () => {
    const topPicks = ref([]);
    const customMajors = ref([]);
    (function load() {
        const sp = localStorage.getItem(getUserKey("top-picks"));
        if (sp)
            topPicks.value = JSON.parse(sp);
        const sc = localStorage.getItem(getUserKey("custom-majors"));
        if (sc)
            customMajors.value = JSON.parse(sc);
    })();
    function savePicks() {
        localStorage.setItem(getUserKey("top-picks"), JSON.stringify(topPicks.value));
    }
    function saveCustom() {
        localStorage.setItem(getUserKey("custom-majors"), JSON.stringify(customMajors.value));
    }
    function addTopPick(pick) {
        topPicks.value.push(pick);
        savePicks();
    }
    function updateTopPick(id, u) {
    ...
    function removeTopPick(id) {
        topPicks.value = topPicks.value.filter((p) => p.id !== id);
        savePicks();
    }
    function addCustomMajor(m) {
        customMajors.value.push(m);
        saveCustom();
    }
    function removeCustomMajor(id) {
